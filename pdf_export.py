...
def font_path():
    ...
    raise RuntimeError("한글 폰트가 없습니다.")
...
def wrap(draw,text,font,width):
    ...
    for para in str(text).splitlines() or [""]:
        cur=""
        for ch in para:
            if draw.textlength(cur+ch,font=font)<=width:cur+=ch
            else:
...
def export_pdf(exam,answers=False):
    ...
    y+=70
    s=f"{len(exam['questions'])}문항 · {exam['total_points']}점 · 엄격 자동검증 통과"
    ...
    for start in range(0,len(qs),2):
        ...
        for side,q in enumerate(qs[start:start+2]):
            ...
            if q.get("conditions"):
                y+=8;d.text((x0,y),"＜조건＞",font=body,fill="black");y+=26
                for c in q["conditions"]:
                    for ln in wrap(d,"○ "+c,small,x1-x0):
                        d.text((x0,y),ln,font=small,fill="black");y+=21

            y+=8;d.text((x0,y),"＜작성 방법＞",font=body,fill="black");y+=26
            sp=q.get("subpoints",[])
            for i,t in enumerate(q.get("tasks",[])):
                prefix="○ "
                # 문제지에는 실제 시험처럼 하위 점수를 굳이 노출하지 않음.
                for ln in wrap(d,prefix+t,small,x1-x0):
                    d.text((x0,y),ln,font=small,fill="black");y+=21

            if answers:
                y+=10;d.text((x0,y),"＜정답·채점 기준＞",font=body,fill="black");y+=26
                ans=q.get("answer",[])
                sol=q.get("solution",[])
                for i,a in enumerate(ans):
                    pts=sp[i] if i<len(sp) else "?"
                    line=f"[{pts}점] {a}"
                    for ln in wrap(d,line,small,x1-x0):
                        d.text((x0,y),ln,font=small,fill="black");y+=21
                    if i<len(sol):
                        for ln in wrap(d,"- "+str(sol[i]),small,x1-x0):
                            d.text((x0,y),ln,font=small,fill="black");y+=21
                if q.get("source_basis"):
                    for ln in wrap(d,"근거: "+q["source_basis"],small,x1-x0):
                        d.text((x0,y),ln,font=small,fill="black");y+=21
        pages.append(im)
